...
//JUnit tests for DoubleLinkedList
...
public class TestDoubleLinkedList extends BaseTest {
    ...
    public void testBasicDelete() {
    	IList<String> list = this.makeBasicList();
        assertEquals("b", list.delete(1)); //deletes from middle
        this.assertListMatches(new String[] {"a", "c"}, list);
    }
    
    @Test(timeout=5 * SECOND)
    public void testBasicDeleteEdgeCases() {
    	IList<String> list = this.makeBasicList();
        assertEquals("a", list.delete(0)); //deletes from front
        this.assertListMatches(new String[] {"b", "c"}, list);
     
        assertEquals("c", list.delete(1)); //deletes from end
    ...
    public void testDeleteUpdatesSize() {
        ...
        for (int i = 9999; i >= 0; i--) {
            condition = i % 5;
            if (condition == 0) { //deletes from front
                list.delete(0);
            } else if (condition == 1) { //deletes second from front
                list.delete(1);
            } else if (condition == 2) { //deletes from middle
                list.delete(i / 2);
            } else if (condition == 3) { //deletes second from end
                list.delete(i - 1);
            } else { //deletes from end
                list.delete(i);
            }    
    ...
    public void testDeleteInvalidInput() {
    	...
    	try {
            ...
            try { //after catching the first exception, checks if invalid input gives the same IOOB exception
                list.delete(-1);
                fail("Expected IndexOutOfBoundsException");
    ...
    @Test(timeout=5 * SECOND)
    public void testDeleteOneOrNone() {
    	IList<String> list = this.makeInstance();
    	list.add("a");
    	
    	assertEquals("a", list.delete(0)); //deletes with 1 item in the list
        this.assertListMatches(new String[] {}, list);
